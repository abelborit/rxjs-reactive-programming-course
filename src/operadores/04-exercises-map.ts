...
/* crear un elemento div con bastante información que me permite hacer scroll */
...
/* seleccionar el elemento donde queremos colocar el texto creado y luego añadirlo */
const divApp = document.getElementById("app");
divApp?.append(createdText);

/* crear un elemento para la barra de progreso, colocarle una clase para los estilos y luego añadirlo en la parte donde queremos que se muestre */
const progressBar = document.createElement("div");
progressBar.setAttribute("class", "progress-bar");
divApp?.append(progressBar);

/* función para calcular el porcentaje del scroll */
const calculatePercentageScroll = (event: Event) => {
  const eventTarget = event.target as Document;

  /* clientHeight: es el alto de lo que somos capaces de desplegar en la pantalla sin hacer scroll */
  /* scrollHeight: es el alto total que podemos hacer scroll, es todo el contenido html que tenemos */
  /* scrollTop: variable que nos ayuda a determinar la posición en la que nos encontramos con respecto del top */
  const { scrollTop, scrollHeight, clientHeight } = eventTarget.documentElement;
  // console.log({ scrollTop, scrollHeight, clientHeight });

  return (scrollTop / (scrollHeight - clientHeight)) * 100;
};

/* crear un observable en base al scroll del documento y al suscribirnos ir seteando el porcentaje que obtengamos */
const progressScroll$ = fromEvent<Event>(document, "scroll");
